...
const Login = async (req, res) => {
  try {
    ...
    // destructring data except password from the user documents.
    const { password, ...users } = user._doc;

    // initializing JWT Token
    const accessToken = jwt.sign(users, process.env.JWT_SEC);

    // setting jwt token in header & sending to the user.
